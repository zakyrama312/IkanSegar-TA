    ...
    if (is_array($keranjang) && !empty($keranjang)) {
        ...
        if ($kembalian >= 0) {
            ...
            if (mysqli_query($koneksi, $query_trx)) {
                ...
                // --- TAMBAHAN LOGIKA AUTO-SIMPAN PELANGGAN KE CRM ---
                if ($nama_pembeli !== 'Pelanggan Umum') {
                    $cek_pembeli = mysqli_query($koneksi, "SELECT id FROM pembeli WHERE nama_pembeli = '$nama_pembeli'");
                    if (mysqli_num_rows($cek_pembeli) == 0) {
                        mysqli_query($koneksi, "INSERT INTO pembeli (nama_pembeli) VALUES ('$nama_pembeli')");
                    }
                }
                // ---------------------------------------------------
...
// Ambil data riwayat pelanggan untuk fitur Auto-suggest (Datalist) DARI TABEL PEMBELI
$q_pelanggan = mysqli_query($koneksi, "SELECT nama_pembeli FROM pembeli ORDER BY nama_pembeli ASC");
?>
...
<body class="bg-gray-100 text-gray-800 antialiased h-screen overflow-hidden flex flex-col">
    ...
    <div id="area-struk" class="hidden text-sm bg-white">
        <div class="text-center mb-4">
            <h2 class="font-bold text-2xl uppercase">SegarLaut.</h2>
